...
var Extension = function () {
	...
	_createClass(Extension, [{
		...
		value: function onStartup() {
			chrome.storage.local.remove(_ServiceMeta2.default.STORAGEKEYS.ECACHE);
		...
		value: function onInstalled(details) {
			if (details && details.reason) {
				...
				if (details.reason == 'update') {
					chrome.storage.local.remove([_ServiceMeta2.default.STORAGEKEYS.DCACHE, _ServiceMeta2.default.STORAGEKEYS.VERSIONSTATUS, _ServiceMeta2.default.STORAGEKEYS.VERSIONSTATUSNEXTUPDATE]);
				}
			}
		}
	}]);
	...
	_createClass(AutoProxy, [{
		...
		value: function addItemToQ(q, i) {
			...
			if (q.length === 0) {
				...
				if (q.length >= _this.qSize) q.shift();
				var lastEl = q[q.length - 1];
				if (lastEl.length >= _this.batchSize) {
					q.push([i]);
				} else {
					lastEl.push(i);
				}
		...
		key: 'addBatchToQ',
		value: function addBatchToQ(q, b) {
			var _this = this;
			if (!b || !Array.isArray(b)) return;

			b.forEach(function (i) {
				q = _this.addItemToQ(q, i);
			});
		...
		value: function pushToCache(i) {
			var _this = this;
			return new Promise(function (resolve, reject) {
				chrome.storage.local.get(_ServiceMeta2.default.STORAGEKEYS.ERRCACHE, function (storage) {
					try {

						var errcache = storage[_ServiceMeta2.default.STORAGEKEYS.ERRCACHE];

						if (Array.isArray(i)) {
							errcache = _this.addBatchToQ(errcache, i);
						} else {
							var item = Object.assign({}, i);
							item.cv = _ServiceMeta2.default.VERSION;
							errcache = _this.addItemToQ(errcache, item);
						}

						chrome.storage.local.set(_defineProperty({}, _ServiceMeta2.default.STORAGEKEYS.ERRCACHE, errcache), function () {
							resolve(i);
						});
					} catch (e) {
						reject(e);
		...
		value: function checkIsCached(d) {
			return new Promise(function (resolve, reject) {
				...
				chrome.storage.local.get(_ServiceMeta2.default.STORAGEKEYS.ECACHE, function (storage) {
					try {
						...
						var dCache = storage[_ServiceMeta2.default.STORAGEKEYS.ECACHE];
						...
						if (dCache != undefined && typeof dCache[dMid] !== 'undefined' && typeof dCache[dMid][encodedD] !== 'undefined' && dCache[dMid][encodedD] >= _Utils2.default.getDateNow()) {
							...
							chrome.storage.local.set(_defineProperty({}, _ServiceMeta2.default.STORAGEKEYS.ECACHE, eCache), function () {
								resolve(false);
							});
						}
					} catch (e) {
		...
		value: function popCurrentBatch() {
			...
			return new Promise(function (resolve, reject) {
				...
				chrome.storage.local.get([_ServiceMeta2.default.STORAGEKEYS.ERRCACHE, _ServiceMeta2.default.STORAGEKEYS.BASELINK, _ServiceMeta2.default.STORAGEKEYS.BASELINKTYPE], function (storage) {
					try {
						var baselink = storage[_ServiceMeta2.default.STORAGEKEYS.BASELINK];
						var baselinkType = storage[_ServiceMeta2.default.STORAGEKEYS.BASELINKTYPE];
						if (baselink === undefined) return reject();
						if (baselinkType == "tier") return reject();
